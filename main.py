...
def generate_rag_response(user_response, chunks_file, encodings_file, conversation_history, phone_number, pdf_file, vector_file, rag_no, use_which_gpu):
    ...
    if rag_no >= 4:
        ...
        if response_type == "connect_agent":
            ...
            key = phone_number
            if key in processing_store:
                processing_store[key]["result"] = "Thank you for contacting us. Currently All the Agents are Busy\nGenerating Ticket --"
            ...
            key = phone_number
            if key in processing_store:
                processing_store[key]["result"] = "Thank you for contacting us."
            
            # Clear stage and end conversation
            clear_stage(phone_number)
            return
        else:
            # User wants to continue troubleshooting
            #rag_no = 0  # Reset rag_no to allow more attempts
            set_stage(
                stage="tech_support",
                phone_number=phone_number,
                com_name=com_name,
            ...
            key = phone_number
            if key in processing_store:
                processing_store[key]["result"] = "Let's try another approach to solve your issue..."
            return
    ...
    if use_which_gpu == '1':
        request = requests.post("http://43.252.194.228:6001/generate", json={"prompt" : str(conversation_history), "text" : user_response})
...
async def messages(req):
    ...
    async def on_turn(turn_context: TurnContext):
        ...
        print(f"From: +91{activity.from_property.name}")
                    ...
                    if get_stage(phone_number) == {}:
                        ...
                        if response.lower() == "yes":
                            ...
                            if vector_file != '0' and chunks_filename != '0':
                                ...
                                conn.commit()
                                set_stage("tech_support", phone_number, com_name=com_name, mo_name=mo_name, user_name=username, pdf_file=pdf_file, vector_file=vector_file, chunks_file=chunks_filename)
                                await turn_context.send_activity(f"Great! I'll use specialized support for your model. What seems to be the problem?")
                                return 
                            
                            else:
                                encodings_process(pdf_file=pdf_file, phone_number=phone_number, com_name=com_name, mo_name=mo_name, username=username)
